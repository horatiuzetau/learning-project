...
create table test_hash_index_table
(
...
create index idx_hash_test_hash_index_table_id
    on test_hash_index_table using hash (id);

-- output: "Index Scan using idx_hash_test_hash_index_table_id on test_hash_index_table"
-- This means that the idx_hash_test_hash_index_table_id index is indeed used and that findById will run in O(1)
explain
select *
...
------------------------------------------------------------
create index idx_hash_test_hash_index_table_uq_indexed
    on test_hash_index_table using hash (uq_indexed);

-- output: "Index Scan using idx_hash_test_hash_index_table_uq on test_hash_index_table"
...
------------------------------------------------------------
create index idx_hash_test_hash_index_table_non_uq_indexed
    on test_hash_index_table using hash (non_uq_indexed);

-- output: "Bitmap Index Scan on idx_hash_test_hash_index_table_non_uq_indexed"
...
-- output: "Seq Scan on test_hash_index_table"
...
------------------------------------------------------------
-- output: Index Scan using test_hash_index_table_uq_key on test_hash_index_table
